...
<div class="tapes promo-train-block__slider-item" style="background: url(<?=$arResult['DETAIL_PICTURE']['SRC']?>) no-repeat; background-size:cover">
	<div class="promo-train-block__slider-item__slider-content">
		<? if ($arResult['USER_FIELDS']['UF_BANNER_TITLE']) : ?>
		<div class="promo-train-block__slider-item__slider-content-tabs">
			<a class="promo-train-block__slider-item__slider-content-tab active" id="" href="#">
				<p class="promo-train-block__slider-item__slider-content__promo-title "><?= $arResult['USER_FIELDS']['UF_BANNER_TITLE'] ?></p>
			</a>
		</div>
		<div class="promo-train-block__slider-item__slider-content-tab__content " id="">
		<p class="promo-train-block__slider-item__slider-content__slogan"><?= $arResult["USER_FIELDS"]['UF_BANNER_NAME'] ?></p>
				<p class="promo-train-block__slider-item__slider-content__dicription"><?=$arResult["DESCRIPTION"] ?></p>
				<div class="promo-train-block__slider__content_bottom">
					<div class="promo-train-block__slider-item-button">
						<a href="<?=$arResult['USER_FIELDS']['UF_BANNER_LINK']?>" class="promo-train-block__slider-item-button__text"><?=$arResult['USER_FIELDS']['UF_BANNER_BUTTON']?></a>
					</div>
				</div>
			</div>
		<? else : ?>
...
<script>
	$(document).ready(function() {
		...
		toggleTabs('.promo-train-block__slider-item__slider-content-tab', '.img-block_img-link');
		toggleTabs('.promo-train-block__slider-item__slider-content-tab', '.together_block');
	});
</script>
...
<div class="bootstrap-box special-offer-block">
	<div class="special-offer-block__title">
		<p class="special-offer-block__title__name">Все товары</p>
	</div>
<? $GLOBALS['arTogether'] = ["SECTION_ID" => $arResult['ID']];?>
<? $APPLICATION->IncludeComponent(
	"bitrix:news.list",
...
<div class="bootstrap-box special-offer-block">
	...
	<? $index = 0;
	foreach ($arResult["ITEMS"] as $cell => $arElement) : ?>
	<div class="together_block <?= $index == 0 ? '' : 'hidden-block' ?>" id="<?= $arElement['ID'] ?>">
	<? $index++; ?>
		<? $GLOBALS['arTogether_'.$arElement['ID']] = ["ID" => $arElement['PROPERTIES']['RECOMMEND']['VALUE']];?>
		<? $APPLICATION->IncludeComponent(
		"bitrix:news.list",
		"special_offer_bundle",
		array(
			...
			"DISPLAY_TOP_PAGER" => "N",
			"FIELD_CODE" => array("ID", "CODE", "XML_ID", "NAME", "TAGS", "SORT", "PREVIEW_TEXT", "PREVIEW_PICTURE", "DETAIL_TEXT", "DETAIL_PICTURE", "DATE_ACTIVE_FROM", "ACTIVE_FROM", "DATE_ACTIVE_TO", "ACTIVE_TO", "SHOW_COUNTER", "SHOW_COUNTER_START", "IBLOCK_TYPE_ID", "IBLOCK_ID", "IBLOCK_CODE", "IBLOCK_NAME", "IBLOCK_EXTERNAL_ID", "DATE_CREATE", "CREATED_BY", "CREATED_USER_NAME", "TIMESTAMP_X", "MODIFIED_BY", "USER_NAME", ""),
			"FILTER_NAME" => "arTogether_".$arElement['ID'],
			"HIDE_LINK_WHEN_NO_DETAIL" => "N",
			"IBLOCK_ID" => "15",
			"IBLOCK_TYPE" => "catalog",
			"INCLUDE_IBLOCK_INTO_CHAIN" => "N",
